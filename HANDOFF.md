...
- Rolagem lateral no celular: em 320–360px ainda passam da tela as fotos dos cards de "Outros modelos" do Torniquete (515px), o shape do card "Em destaque"/"Lançamento" (296px) e o `.site-footer__desc` (298px). Todas são partes comuns; aguardando o ok do usuário para corrigir. No Contato, entre 768px e 1023px, o botão "Ver no maps" da seção `.mapa` passa da tela (171px em 768, 100px em 844×390). A causa é o endereço com CEP em `white-space: nowrap`, que não encolhe. O chat do Contato já testou a correção, só no `contato.css` e sem mudar nada em 1440px, e ela espera o ok do usuário.
