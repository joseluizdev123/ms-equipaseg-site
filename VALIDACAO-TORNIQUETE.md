...
Altura total: Figma 8051,12px, site 8170px (ver diferenças). O hero começa na mesma linha do Figma, mas ficou 13px mais alto: a linha de botões passou de 59px para 72px com os botões no padrão do site. A faixa "Nossos clientes", que não existe no frame, soma mais 104px, os textos novos dos modelos tiram 8px e o título de 40px da "Instalações" soma 10px. Por isso a faixa começa 13px abaixo do fim do hero do Figma, as seções seguintes descem 117px, depois dos modelos 109px e, depois da "Instalações", 119px. As divergências por seção acima são de 11/09/2026: diferenciais, "Em destaque", modelos e "Instalações" mudaram em 13/09/2026 e não foram medidos de novo.
...
| Card alto dos diferenciais | preto, com ícone no alto e um parágrafo no pé: "Construção estrutural totalmente em alumínio com pintura eletrostática, para ambientes de atmosfera agressiva (Região litorânea)." | card com foto (`bento-card--foto`, no `main.css`): `produto-torniquete-md300.jpg` no alto, com fundo `--color-media-dark` igual ao da foto; no pé, título "Estrutura em alumínio" (Exo 20/27, branco, regra no `torniquete.css`) e "Pintura eletrostática para atmosfera agressiva, como regiões litorâneas." em `--gray-03` | pedido do usuário (13/09/2026): o card tinha um vazio grande entre o ícone e o texto. O card manteve 416px. O mesmo card com foto foi para o card alto do Projetos (parte comum) |
| Seção "Instalações" | fundo claro, título de 32px e 6 itens provisórios (galpão, centro de distribuição, aeroporto, prédio e dois cards com texto) | igual à do Dilacerador: `galeria galeria--escura` (fundo escuro), título `titulo-secao` de 40px, setas `icon-chevron-left-dim.svg` e `icon-chevron-right.svg`; as 13 fotos da cliente, só foto, sem card: `instalacao-01.jpg` a `-13.jpg`, a mesma lista do Dilacerador e dos Totens | pedido do usuário, feito pelo Mestre (13/09/2026). Com a página parada em 1440px, a seção muda de cor e mostra outras fotos. O título maior deixou a seção 10px mais alta (887px → 897px) |
...
| Botão dos cards de modelo | "Baixar manual" | "Baixar informações" com seta, `cta cta--xs` de 188px como no Dilacerador (13/09/2026); abre a lista Manual, Catálogo e Infraestrutura civil e elétrica | pedido do usuário em 12/09/2026, feito pelo chat do Dilacerador nas 3 páginas com cards de modelo. "Catálogo" abre `assets/docs/catalogo-torniquetes.pdf` numa aba nova, nos dois cards (Mestre, 13/09/2026); Manual e Infraestrutura civil e elétrica ficam em `#` até chegarem os arquivos |
...
Sem rolagem lateral em 1440, 1425 (com a barra de rolagem), 1366, 1280, 1024, 768 e 390px. Os cards dos diferenciais ficam numa grade de 3 colunas que encolhe até 1024px, onde passa a 2 colunas; até 767px, uma coluna. Os cards de modelos empilham a partir de 1023px. O card com foto dos diferenciais ocupa a largura toda a partir de 1023px, com a foto no tamanho do desktop (296×268) centralizada; ele não causa rolagem lateral em 768 nem em 390px (13/09/2026).

## Smoke test

Console sem erros. Ícones e fotos novos: `icon-escudo.svg`, `icon-whatsapp-24.svg`, `shape-foto.svg` e as fotos `torniquete-*` em `assets/seed/`. O `icon-escudo-claro.svg` saiu da página em 13/09/2026, com o card com foto, que usa `produto-torniquete-md300.jpg`.
