...
| 6 | Instalações | `5241:591` | — | 0, 0 | desde 13/09 são as 13 fotos da cliente (`instalacao-01` a `-13`), iguais nas galerias de Instalações do Dilacerador, do Torniquete e dos Totens; como todas diferem do Figma, a divergência por pixel deixou de servir; com as fotos do Figma a seção dava 0,59% |
...
| Galeria "Instalações" | 6 fotos, 4 com escurecimento e 2 com card de texto | 13 fotos da cliente (`instalacao-01.jpg` a `-13.jpg`), as mesmas do Torniquete e dos Totens, sem escurecimento e sem card; os 4 pontinhos do Figma distribuídos pelas 13 | pedido do usuário, feito pelo Mestre em 13/09 |
...
| Cabeçalho ao rolar | rola junto com a página | fica fixo no topo; o "Saber mais" leva a "Modelos disponíveis" logo abaixo dele, e a lista do "Baixar informações" passa por baixo | parte comum, pedido do usuário (14/09) |
| Letras no celular (até 767px) | só desktop | escala do site atual: 48→32, 40→28, 32→24, 24→20, 18→16 e 16→15px, com line-height proporcional; bloco com `:root` no fim do `main.css` | parte comum, pedido do usuário (14/09); em 1440px nada muda |
| Altura do título dos modelos no celular | 43px fixos | até 767px a altura acompanha o texto (32px com uma linha); em 360px "Lombada – Garra dupla" quebra em duas linhas (64px) sem cobrir a descrição | correção desta página (14/09); em 1440px os títulos continuam com 43px |
...
| Textos dos modelos | Embutido com o texto do Figma; Garra dupla com "Descrição" | Embutido com o texto oficial do Unidirecional; "Lombada – Garra dupla" com o texto oficial do Bidirecional | o usuário considera que são os mesmos produtos. Confirmar com a MS Equipaseg: o texto do Unidirecional diz 100% mecânico, mas o Embutido aparece como elétrico no site atual ("atua em 1 segundo") e no catálogo da cliente (`catalogo-dilacerador-de-pneus.pdf`, V2: acionamento por botoeira, controle de acesso, remotamente ou por laço indutivo, para o Embutido e a Lombada) |
...
| Botão dos cards de modelo | "Baixar manual", 142px | "Baixar informações" com seta, 188px, que abre a lista Manual, Catálogo e Infraestrutura civil e elétrica | pedido do usuário, igual no Torniquete e no Controle de acesso. Desde 13/09, "Catálogo" abre o PDF da cliente numa aba nova (`docs/catalogo-dilacerador-de-pneus.pdf`); Manual e Infraestrutura continuam `#`, sem arquivo |
...
- Arquivos novos: fotos em `seed/` (`dilacerador-hero.jpg`, `instalacao-01.jpg` a `-13.jpg` (conjunto comum às galerias de Instalações das três páginas de produto) (as antigas `instalacao-cancela.jpg` e `instalacao-cancela-2.jpg` ficaram no `seed/`, fora de uso), `aplicacao-*.jpg`, `modelo-embutido.png`, `modelo-lombada.png`) e SVGs em `images/` (`icon-fence.svg`, `icon-ruler.svg`, `icon-wrench.svg`, `icon-arrow-down-up.svg`, `icon-bell-electric.svg`, `icon-app-window-mac.svg`, `icon-car.svg`, `icon-siren.svg`, `icon-cctv.svg`, `icon-circuit-board.svg`, `icon-chevron-left-dim.svg`, `icon-chevron-down-gray.svg`, `cta-shape-white-160.svg`, `cta-shape-142.svg`, `pattern-elipses-modelo.svg`, `faq-ilustracao.svg`).
